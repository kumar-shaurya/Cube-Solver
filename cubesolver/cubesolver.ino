...
<body>
  ...
  <script>
    ...
    let paintState = new Array(54).fill(0);
        ...
        for(let x=-1; x<=1; x++) {
            for(let y=-1; y<=1; y++) {
                for(let z=-1; z<=1; z++) {
                    ...
                    let faces = [
                        { axis: 'y', val: -1, color: colors[0], class: 's-U' },
                        { axis: 'x', val: 1,  color: colors[1], class: 's-R' },
                        { axis: 'z', val: 1,  color: colors[2], class: 's-F' },
                        { axis: 'y', val: 1,  color: colors[3], class: 's-D' },
                        { axis: 'x', val: -1, color: colors[4], class: 's-L' },
                        { axis: 'z', val: -1, color: colors[5], class: 's-B' }
                    ];

                    faces.forEach(f => {
                        let sticker = document.createElement('div');
                        sticker.className = 'sticker ' + f.class;
    ...
    // --- 3D ANIMATED ROTATION LOGIC ---
    function animateMove(move, duration) {
        return new Promise(resolve => {
            isAnimating = true;
            let base = move[0];
            let modifier = move.length > 1 ? move[1] : '';
            let dir = modifier === "'" ? -1 : (modifier === "2" ? 2 : 1);
            let axis, value, angle;

            if(base==='U') { axis='y'; value=-1; angle= -90 * dir; }
            if(base==='D') { axis='y'; value= 1; angle=  90 * dir; }
            if(base==='R') { axis='x'; value= 1; angle=  90 * dir; }
            if(base==='L') { axis='x'; value=-1; angle= -90 * dir; }
            if(base==='F') { axis='z'; value= 1; angle=  90 * dir; }
            if(base==='B') { axis='z'; value=-1; angle= -90 * dir; }

            let pivot = document.getElementById('pivot');
            let moving = cubies.filter(c => c.logical[axis] === value);
            
            // Attach active slice to pivot
            moving.forEach(c => pivot.appendChild(c));

            if(duration > 0) {
                pivot.style.transition = `transform ${duration}ms ease-in-out`;
            } else {
            ...
            setTimeout(() => {
                ...
                moving.forEach(c => {
                    // Prepend rotation matrix to bake it into the cubie safely
                    let rotStr = `rotate${axis.toUpperCase()}(${angle}deg) `;
                    let currentTrans = c.style.transform.replace('none', '');
                    c.style.transform = rotStr + currentTrans;
                    document.getElementById('cube').appendChild(c);
                    
                    // Update logical 3D coordinates
                    let x = c.logical.x, y = c.logical.y, z = c.logical.z;
                    let rad = angle * Math.PI / 180;
                    let cos = Math.round(Math.cos(rad)), sin = Math.round(Math.sin(rad));
                ...
                isAnimating = false;
                resolve();
            }, duration + 20); // small buffer to ensure CSS completes
        });
    }

    // --- DRAG TO ROTATE CUBE VIEW ---
    let isDragging = false, prevX = 0, prevY = 0, rotX = -20, rotY = -45;
    ...
    function moveDrag(e) {
        ...
        rotY += (x - prevX) * 0.5; rotX -= (y - prevY) * 0.5;
    ...
    async function stepForward() {
        if(isAnimating || currentStep >= solutionMoves.length) return;
        document.getElementById('currentMoveDisplay').innerText = "Executing: " + solutionMoves[currentStep];
        await animateMove(solutionMoves[currentStep], 300);
        currentStep++;
        updatePlayerUI();
    }
    ...
    async function stepBackward() {
        ...
        document.getElementById('currentMoveDisplay').innerText = "Reversing: " + move;
        // Invert the move to go backward
        let inv = move.includes("'") ? move[0] : (move.includes("2") ? move : move + "'");
        await animateMove(inv, 300);
        updatePlayerUI();
    }
    ...
    async function togglePlay() {
        ...
        if(autoPlayInterval) {
            ...
            }, 600); // Wait 600ms between moves
        }
    }

    function updatePlayerUI() {
        document.getElementById('btnPrev').disabled = (currentStep === 0);
        document.getElementById('btnNext').disabled = (currentStep === solutionMoves.length);
        if(currentStep === solutionMoves.length && solutionMoves.length > 0) {
            document.getElementById('currentMoveDisplay').innerText = "Cube Solved! 🎉";
        }
    }

    // --- UI TOGGLES & BASIC LOGIC ---
    function toggleMode() {
    ...
    async function userMove(m) {
        ...
        await animateMove(m, 300); // Smooth animation for user moves
    }
    ...
    async function generateScramble() {
        ...
        for(let i=0; i<20; i++) {
            ...
            currentScramble.push(m);
            await animateMove(m, 0); // Instant execution for scramble prep
        }
        document.getElementById("scrambleStr").innerText = currentScramble.join(" ");
    }
    ...
    function clearAll() {
        ...
        for(let i=0; i<54; i++) paintState[i] = Math.floor(i/9); // Reset paint
        init3DCube();
        drawPaintCanvas();
    }
    ...
    function solveCube() {
      ...
      if(isPaintMode) {
          let cubeString = paintState.map(val => faceLetters[val]).join('');
          payload = { cube_string: cubeString };
      } else {
          if(currentScramble.length === 0) { document.getElementById("solveBtn").disabled = false; return alert("Scramble first!"); }
      ...
      .then(async data => {
        ...
        if(isPaintMode) {
            document.getElementById("view2D").classList.add("hidden");
            document.getElementById("view3D").classList.remove("hidden");
            init3DCube(); // Fresh cube
            if(data.scramble) {
                // Instantly build the user's painted cube in 3D using the Python scramble math!
                for(let m of data.scramble.split(" ")) await animateMove(m, 0);
            }
        }
    ...
    document.getElementById('cubeCanvas').addEventListener('mousedown', function(e) {
        ...
        for (let f = 0; f < 6; f++) {
            ...
            for (let i = 0; i < 9; i++) {
                let x = ox + ((i % 3) * (sq + gap)); let y = oy + (Math.floor(i / 3) * (sq + gap));
                if (clickX >= x && clickX <= x + sq && clickY >= y && clickY <= y + sq) {
                    paintState[f * 9 + i] = (paintState[f * 9 + i] + 1) % 6;
                    drawPaintCanvas();
                    return;
                }
            }
    ...
    function drawPaintCanvas() {
        ...
        for (let f = 0; f < 6; f++) {
            ...
            for (let i = 0; i < 9; i++) {
                ...
                ctx.fillRect(x, y, sq, sq); ctx.strokeStyle = "#000"; ctx.strokeRect(x, y, sq, sq);
            }
        }
    }
